...
const addRole = () => {
	db.showDepartments().then(([data]) => {
		...
		db.addRole(roleName, roleSalary, roleDepartment).then(() => console.log (`Successfully added ${roleName, roleSalary, roleDepartment}`)).then(() => promptUser())
		})
	})
};
...
const addEmployee = () => {
	...
	db.showRoles().then(([data]) => {
		...
		.then(({employeeFirstName, employeeLastName, employeeRole, employeeManager}) => {
			db.addEmployee(employeeFirstName, employeeLastName, employeeRole, employeeManager).then(() =>
		...
		.then(({employeeId}) => {
			db.getManagers(employeeId).then(([data]) => {
				...
				inquirer.prompt([
					...
					db.updateManager(managerId, employeeId).then(() => {
		...
		inquirer.prompt([
			{
				type: "list",
				name: "employeeId",
				message: "Which employee would you like to delete? You cannot undo this!",
				choices: employeeChoices
			}
		])
		.then(({employeeId}) => {
